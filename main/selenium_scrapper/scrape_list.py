...
def init_data_scrapping(selenium_web_driver):
    ...
    while page <= total_pages:
        scrapped_data_list.extend(
            try_scrapping(selenium_web_driver, total_items)
        )
        if load_next_page_is_available(selenium_web_driver):
            page = scrape_data(page, scrapped_data_list, selenium_web_driver, total_items)
        elif load_next_page_is_available(selenium_web_driver):
            page = scrape_data(page, scrapped_data_list, selenium_web_driver, total_items)
        else:
            break
...
def scrape_data(page, scrapped_data_list, selenium_web_driver, total_items):
    page += 1
    print(f'page = {page} [ {len(scrapped_data_list)} ]', end=' ')

    while len(scrapped_data_list) == 0:
        scrapped_data_list.extend(
            try_scrapping(selenium_web_driver, total_items)
        )
...
def try_scrapping(selenium_web_driver, total_of_elements):
    ...
    try:
        ...
        tb = sys.exc_info()[0]
        print(f"Scrape Data TraceBack {tb}")
        if len(scrapped_data_list) == 0 and total_of_elements > 0:
            time.sleep(5)
            try_scrapping(selenium_web_driver, total_of_elements)
